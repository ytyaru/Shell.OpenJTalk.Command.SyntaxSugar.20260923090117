...
OJT_LIST_PY = os.path.abspath(os.path.join(SCRIPT_DIR, "..", "list", "list.py"))
        ...
        for line in f:
            ...
            if len(parts) >= 3:
                ...
                config[spk] = {
                    ...
                    # 💡 修正: 配列内では $NONE というリテラル文字列をそのまま維持する
...
def cmd_get_model(speaker, tone, config):
    ...
    delim = config[speaker]["delim"]
    # 💡 組み立てる直前で、結合子や調子の中にある $NONE を空文字へとクリーン化する
    actual_delim = "" if delim == "$NONE" else delim
    actual_tone = "" if tone == "$NONE" else tone
    return f"{speaker}{actual_delim}{actual_tone}"
...
def main():
    ...
    if subcommand == "get":
        ...
        if len(sys.argv) < 4: print("No"); sys.exit(1)
        ...
        if len(sys.argv) < 3 or query not in config:
            print("エラー: 指定話者は調子差異がありません", file=sys.stderr)
            sys.exit(1)
        for t in config[query]["tones"]:
            print("" if t == "$NONE" else t)
