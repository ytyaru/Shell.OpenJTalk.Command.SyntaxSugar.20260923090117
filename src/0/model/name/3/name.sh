...
cmd_has() {
    ...
    # 実ファイル存在チェックレイヤーを正確に呼び出す
    (cd "${SCRIPT_DIR}" && "${OJT_LIST}" has "${model}")
}
...
cmd_listTalker() {
    { grep -v '^#' "${TSV_FILE}" | cut -f1; (cd "${SCRIPT_DIR}" && "${OJT_LIST}" list) | while read -r v; do
        local matched=0
...
cmd_countTone() {
    # パイプで wc -l に渡すとエラーでも 0 が出力されてしまうため、先に話者チェックを行う
    _require "$1" "話者名"
    local line
    line="$(_getLine "$1")"
    [ -n "$line" ] || { echo "エラー: 指定話者は調子差異がありません" >&2; exit 1; }
    # 存在する話者であれば、安全に件数を出力する
