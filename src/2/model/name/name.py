...
def load_config_sh():
    ...
    if match: 
        # 💡 末尾のスラッシュは config.sh の定義をそのまま尊重する
...
def get_all_list_models():
    # 💡 cwd=SCRIPT_DIR を指定して、実行場所による相対パスの崩壊を防ぐ
    res = subprocess.run([sys.executable, OJT_LIST_PY, "list"], capture_output=True, text=True, check=True, cwd=SCRIPT_DIR)
...
def cmd_has(speaker, tone, config):
    try:
        ...
        # 💡 ここも cwd を指定
        res = subprocess.run([sys.executable, OJT_LIST_PY, "has", model], cwd=SCRIPT_DIR)
